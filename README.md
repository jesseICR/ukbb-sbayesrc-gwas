...
- **Per-block eigen-decomposed LD matrices** (Steps 7–9) — the four `SBayesRC::LDstep*` R functions are run on DNAnexus (one SAK job for LDstep1, 22 per-chromosome SAK jobs for LDstep2+3, one SAK job for LDstep4), producing `block{i}.eigen.bin` + `ldm.info` + `snp.info` — the exact directory layout that downstream SBayesRC (`tidy`, `impute`, `sbayesrc`) expects as its `LDdir`.

> ### Caveat: Steps 7–9 have not been verified end-to-end
>
> Steps 1–7 of the pipeline have run through successfully at least once each and their outputs have been inspected. **Steps 8 and 9 have never completed a full run.** Step 8 (the per-chromosome LDstep2 + LDstep3 jobs) was mid-flight when UK Biobank suspended DNAnexus access on 2026-04-23 — see [*A message to our participants: UK Biobank data security update*](https://www.ukbiobank.ac.uk/news/a-message-to-our-participants-uk-biobank-data-security-update/). The last confirmed block-level event was chr1's first block completing `LDstep2` (gctb `--make-full-ldm --thread 16`, m=7323 SNPs, 3:18 wall) and entering `LDstep3` (eigen decomposition). Before that, four distinct bugs were found and fixed iteratively — a stale GCTB download URL, a SAK auto-upload slowdown on the 591 per-block snplist files (now tarred into `snplist.tar.gz`), dash's parsing of SBayesRC's bash-specific `&>` redirection in `ld.sh` (now rewritten to POSIX `> file 2>&1`), and gctb's `--make-full-ldm` defaulting to single-threaded (now invoked with `--thread 16`). So while the scripts build on an iterated understanding of the failure modes, **Step 8's behaviour across all 591 blocks on all 22 chromosomes, Step 9's LDstep4 merge, and the final-output consumability are all unverified**. When DNAnexus access is restored, the pipeline's per-block idempotency should allow `bash generate_ld.sh` to resume exactly where it left off: `dx ls $DX_OUTPUT_DIR/ld_reference/ld_matrices/ | grep -c eigen.bin` will show how many of the 591 blocks already uploaded their eigen file successfully before the cut-off.
...
7. **Initialize the LD-matrix workspace** (`init_ld_matrices.sh` + `sbayesrc_sak_setup.sh`)
   Synthesizes a dummy `dummy_ma.ma` file locally from the QC-passed CSV (columns `SNP A1 A2 freq b se p N`; only `SNP`/`A1`/`A2` are actually used by LDstep1 for allele alignment against the bim files — the summary-statistics columns are present but immaterial for LD construction), uploads it to `$DX_OUTPUT_DIR/ld_reference/ld_matrices/dummy_ma.ma`, and submits a single Swiss Army Knife job that runs `SBayesRC::LDstep1`. Inside the SAK job, the shared helper `sbayesrc_sak_setup.sh` installs R package SBayesRC v0.2.6 from its official GitHub release tarball (plus GCTB v2.04.3, needed by LDstep2 in Step 8). LDstep1 reads the 22 per-chromosome bim files via `/mnt/project`, intersects them against the ma file's SNP list, aligns alleles, groups SNPs into the 591 hg38 blocks, and writes `ld.sh`, `ldm.info`, and 591 per-block snplist files into `$DX_OUTPUT_DIR/ld_reference/ld_matrices/`. The snplists are bundled into a single `snplist.tar.gz` before SAK's auto-upload because uploading 591 tiny files individually is API-call-bound.
8. **Build per-block LD matrices and eigen-decompose** (`build_ld_matrices.sh`)
   Downloads `ldm.info` locally to plan per-chromosome block assignments, then submits 22 parallel Swiss Army Knife jobs (one per chromosome). Each job stages the LDstep1 outputs into a writable local `ld_matrices/` directory (extracting `snplist.tar.gz`), patches the generated `ld.sh` with `sed` to (1) add `--thread 16` to every `gctb` invocation — otherwise gctb runs single-threaded and each block takes 30+ minutes instead of a few — and (2) rewrite bash-specific `cmd &> file` redirection to POSIX `cmd > file 2>&1` (R's `system()` dispatches to `/bin/sh` = dash on Ubuntu, which parses `&>` as background-and-stdout-redirect and returns before gctb finishes). The job then loops over every block assigned to its chromosome and runs `SBayesRC::LDstep2` (which invokes `gctb --bfile ... --make-full-ldm --thread 16` under the hood, producing `b{i}.ldm.full.{bin,info}`) followed by `SBayesRC::LDstep3` (pure-R eigen decomposition, respecting `OMP_NUM_THREADS=16`, writing `block{i}.eigen.bin`). The large `b{i}.ldm.full.bin` files (~600 MB/block) are deleted inside the worker as soon as their eigen decomposition completes; the much smaller `b{i}.ldm.full.info` files + per-block log files are bundled into `chr{N}_meta.tar.gz` and uploaded alongside the individual `block{i}.eigen.bin` files. Per-block idempotency is handled inside the SAK job by a `dx ls` check against `/mnt/project` — a re-submit after a mid-flight failure will resume exactly where it left off, without recomputing eigen decompositions already on DNAnexus.
9. **Merge per-block LD info into `snp.info`** (`merge_ld_info.sh`)
   Submits one final Swiss Army Knife job that stages `ldm.info` plus every `b{i}.ldm.full.info` file (by extracting the 22 `chr{N}_meta.tar.gz` tarballs Step 8 produced), runs `SBayesRC::LDstep4` to concatenate them into a single per-SNP `snp.info` file (columns `Chrom ID Index GenPos PhysPos A1 A2 A1Freq N Block`), and uploads it to `$DX_OUTPUT_DIR/ld_reference/ld_matrices/snp.info`. After Step 9 the LD directory contains everything SBayesRC needs as `LDdir`: `ldm.info`, `snp.info`, and one `block{i}.eigen.bin` per retained block.
...
- **`$DX_OUTPUT_DIR/ld_reference/ld_matrices/`** — the custom SBayesRC LD reference directory, suitable as `LDdir` for downstream `SBayesRC::tidy` / `::impute` / `::sbayesrc` calls. Contains `ldm.info`, `snp.info`, and one `block{i}.eigen.bin` per retained block, plus the LDstep1 workspace (`ld.sh`, `snplist/`, `b*.ldm.full.info`) for reproducibility.
...
DNAnexus writes happen in Steps 1, 4, 5 (upload only), 6, 7, 8, and 9. Steps 2, 3, and the local portion of Step 5 are local only. Steps 1–6 on the default thresholds complete in ~30–45 minutes total; Steps 7–9 (the SBayesRC LD-matrix build) add several hours dominated by the per-block eigen decompositions in Step 8 — with `OMP_NUM_THREADS=16` on `mem2_ssd1_v2_x16` workers, Step 8's 22 parallel per-chromosome jobs typically take 2–7 hours each, with total wall clock set by the slowest chromosome. Subsequent re-runs skip every completed step or block.
